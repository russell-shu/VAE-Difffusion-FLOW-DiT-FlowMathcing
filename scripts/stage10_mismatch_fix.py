"""Stage 10: training-inference mismatch fix (paper eq. 6-7) + CFG sampling."""
...
def main() -> None:
    ...
    logger = setup_logger("stage10-mismatch", run_dir)
    ...
    step = 0
    pbar = tqdm(total=cfg.train.max_steps, desc="stage10")
        ...
        for wave, texts in loader:
            ...
            if step % cfg.train.log_every == 0:
                logger.info(
                    ...
                    f"fix_mismatch={int(cfg.solver.fix_mismatch)} "
                    f"drop_prompt_uncond={int(cfg.solver.drop_noisy_prompt_in_uncond)}"
                )
            ...
            if step % cfg.train.sample_every == 0:
                with torch.no_grad():
                    ...
                    z_hat = solver.integrate(
                        ...
                        guidance=lambda vc, vu, z, tt: cfg_velocity(vc, vu, alpha=cfg.sample.cfg_alpha),
                    )
    ...
    save_checkpoint(
        run_dir / "dit_mismatch.pt",
        {"dit": dit.state_dict(), "refine": text_enc.refine.state_dict(), "cfg": dict(cfg)},
    )
    logger.info(f"saved {run_dir / 'dit_mismatch.pt'}")
