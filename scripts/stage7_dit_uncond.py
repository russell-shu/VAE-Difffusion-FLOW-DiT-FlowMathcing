...
def main() -> None:
    ...
    logger = setup_logger("stage7-dit-uncond", run_dir)
    ...
    if ckpt_path.exists():
        ...
        logger.warning(
            f"Wav-VAE checkpoint not found at {ckpt_path}. "
            "Run stage 6 first; continuing with randomly initialised VAE (debug only)."
        )
    vae.eval()
    ...
    optim = torch.optim.AdamW(dit.parameters(), lr=cfg.optim.lr)

    step = 0
    pbar = tqdm(total=cfg.train.max_steps, desc="stage7")
    while step < cfg.train.max_steps:
        for batch in loader:
            batch = batch.to(device)
            with torch.no_grad():
                z1 = encode_latent(vae, batch)
            ...
            if step % cfg.train.sample_every == 0:
                with torch.no_grad():
                    ...
                    z_s = solver.integrate(lambda z, tt: dit(z, tt)[0], z0=z0[:1])
                    wave = vae.decode(z_s)
                    save_waveform(wave[0, 0], run_dir / f"sample_{step:06d}", sample_rate=cfg.data.sample_rate)

            if step >= cfg.train.max_steps:
                break
    pbar.close()

    save_checkpoint(run_dir / "dit.pt", {"dit": dit.state_dict(), "cfg": dict(cfg)})
    logger.info(f"saved {run_dir / 'dit.pt'}")
