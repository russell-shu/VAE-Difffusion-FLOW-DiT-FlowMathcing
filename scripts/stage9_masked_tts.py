"""Stage 9: masked latent conditioning (VoiceBox-style) + CFG dropout training."""
...
def main() -> None:
    ...
    logger = setup_logger("stage9-masked-tts", run_dir)
    ...
    step = 0
    pbar = tqdm(total=cfg.train.max_steps, desc="stage9")
        ...
        for wave, texts in loader:
            ...
            if step % cfg.train.sample_every == 0:
                with torch.no_grad():
                    ...
                    z_ctx0 = z1[:1] * (1.0 - m[:1])
                    z0 = RectifiedFlow.draw_prior(z1[:1].shape, device=device)
                    solver = EulerSolver(num_steps=cfg.sample.nfe)

                    def vel(z, t):
                        x_in = torch.cat([z, z_ctx0], dim=1)
                        return dit(x_in, t, cross_ctx=q0, cross_mask=cross_attn_mask(pad0))[0]

                    z_hat = solver.integrate(vel, z0=z0)
