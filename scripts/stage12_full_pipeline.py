"""Stage 12: REPA + full inference stack (APG + mismatch fix) + cheap metrics."""
...
from mini_audiodit.eval import evaluate_sample
...
from mini_audiodit.models.repa import MHuBERTExtractor, REPALoss
...
def main() -> None:
    ...
    logger = setup_logger("stage12-full", run_dir)
    ...
    try:
        hubert = MHuBERTExtractor(
            model_name=cfg.repa.model_name,
            target_sr=int(cfg.repa.target_sr),
            layer=int(cfg.repa.layer),
        ).to(device)
        repa_loss = REPALoss(dit_dim=cfg.dit.dim, hub_dim=hubert.model.config.hidden_size).to(device)
        logger.info("REPA: HuBERT features enabled")
    except Exception as e:
        hubert = None
        repa_loss = None
        logger.warning(f"REPA disabled (could not load HuBERT): {e}")

    params = list(dit.parameters()) + list(text_enc.refine.parameters())
    if repa_loss is not None:
        params += list(repa_loss.parameters())
    ...
    step = 0
    pbar = tqdm(total=cfg.train.max_steps, desc="stage12")
    while step < cfg.train.max_steps:
        for wave, texts in loader:
            ...
            if hubert is not None and repa_loss is not None:
                v_pred, repa_feat = dit(
                    x_t, tt, cross_ctx=q, cross_mask=cmask, repa_layer=int(cfg.repa.dit_layer),
                )
            else:
                v_pred = dit(x_t, tt, cross_ctx=q, cross_mask=cmask)
            diff = (v_pred - v_tgt).pow(2) * (1.0 - m)
            denom = (1.0 - m).sum().clamp_min(1.0)
            loss_fm = diff.sum() / denom

            if hubert is not None and repa_loss is not None:
                hub = hubert(wave, sample_rate=cfg.data.sample_rate)
                loss_r = repa_loss(repa_feat, hub)
                loss = loss_fm + float(cfg.repa.weight) * loss_r
            else:
                loss_r = wave.new_zeros(())
                loss = loss_fm
            ...
            if cfg.train.grad_clip is not None:
                ...
                if repa_loss is not None:
                    torch.nn.utils.clip_grad_norm_(repa_loss.parameters(), cfg.train.grad_clip)
            optim.step()
            ...
            if step % cfg.train.log_every == 0:
                logger.info(
                    f"step={step} loss={loss.item():.4f} fm={loss_fm.item():.4f} repa={loss_r.item():.4f}"
                )

            if step % cfg.train.eval_every == 0:
                with torch.no_grad():
                    metrics = evaluate_sample(vae.decode(z1[:1]), wave[:1])
                    logger.info(f"step={step} recon_metrics={metrics}")
            ...
            if step % cfg.train.sample_every == 0:
                with torch.no_grad():
                    ...
                    metrics = evaluate_sample(audio, wave[:1])
                    logger.info(f"step={step} sample_metrics={metrics}")

            if step >= cfg.train.max_steps:
                break
    pbar.close()

    payload = {"dit": dit.state_dict(), "refine": text_enc.refine.state_dict(), "cfg": dict(cfg)}
    if repa_loss is not None:
        payload["repa"] = repa_loss.state_dict()
    save_checkpoint(run_dir / "full.pt", payload)
    logger.info(f"saved {run_dir / 'full.pt'}")
